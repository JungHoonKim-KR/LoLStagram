...
//        return getApiResponseOneData(LOL.BaseUrlAsia, "/riot/account/v1/accounts/by-riot-id/" + riotIdGameName + "/" + riotIdTagline, "puuid", "라이엇 이름 또는 태그가 일치하지 않습니다.");
//    }
//
//    public String getSummonerId(String puuId) {
//        return getApiResponseOneData(LOL.BaseUrlKR, "/lol/summoner/v4/summoners/by-puuid/" + puuId, "id", "소환사 아이디를 찾을 수 없습니다. 라이엇 이름 또는 태그가 일치하지 않습니다.");
//    }
    // 최근 대전기록 가져오기
...
//        return createWebClient(LOL.BaseUrlAsia, Url).bodyToMono(List.class).block();
//    }
...
//        int result = -1;
//        String targetMatchId = getMatches(puuId, LOL.gameCount - 1, 1).get(0);
...
//        List<String> matches = getMatches(summonerInfo.getPuuId(), startGame, count);
//        DecimalFormat df = getDecimalFormat();
//
//        long totalkill = 0, totaldeath = 0, totalassist = 0;
//        for (String matchId : matches) {
//            String origin = getApiResponse(LOL.BaseUrlAsia, "/lol/match/v5/matches/" + matchId, ErrorCode.NOTFOUND, "경기를 찾을 수 없습니다.");
...
//        DecimalFormat df = getDecimalFormat();
//        ObjectMapper mapper = new ObjectMapper();
//        String block = getApiResponse(LOL.BaseUrlKR, "/lol/league/v4/entries/by-summoner/" + summonerId,
